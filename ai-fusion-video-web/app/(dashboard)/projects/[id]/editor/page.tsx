...
import { ArrowLeft, Captions, Clapperboard, Copy, GripVertical, Loader2, Music, Pause, Play, RotateCcw, Save, Scissors, Sparkles, Trash2, Upload, Volume2, X, ZoomIn, ZoomOut } from "lucide-react";
...
type Draft = { version: 1; clips: Clip[]; settings: Settings; templateId?: string; storySeed?: string; updatedAt: string };
...
export default function VideoEditorPage() {
  ...
  const [templateId, setTemplateId] = useState(""); const [storySeed, setStorySeed] = useState("");
  const audioInputRef = useRef<HTMLInputElement>(null);
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }));
  const selected = clips.find(c => c.id === selectedId) || clips[0] || null;
  const activeTemplate = (project?.properties?.videoTemplateSnapshot as VideoTemplate | undefined)
  ...
  useEffect(() => { if (!episodeId) { setLoading(false); return; } void (async () => {
    try { const [ep, scenes] = await Promise.all([storyboardApi.getEpisode(episodeId), storyboardApi.listScenesByEpisode(episodeId)]);
      ...
      const projectTemplateId = typeof project?.properties?.videoTemplateId === "string" ? project.properties.videoTemplateId : "";
      const template = (project?.properties?.videoTemplateSnapshot as VideoTemplate | undefined) || getVideoTemplate(projectTemplateId);
      const templateSettings = template ? { ...defaults, bgmUrl: template.audio.bgmUrl || "", bgmVolume: template.audio.bgmVolume, originalAudioVolume: template.audio.originalAudioVolume } : defaults;
      setEpisode(ep); setClips(draft?.clips || initial); setSettings(draft?.settings || templateSettings); setTemplateId(projectTemplateId || draft?.templateId || ""); setStorySeed(draft?.storySeed || ""); setSavedAt(draft?.updatedAt); setSelectedId((draft?.clips || initial)[0]?.id || null);
    } finally { setLoading(false); }
  })(); }, [episodeId, project?.properties?.videoTemplateId, storageKey]);

  function save() { const updatedAt = new Date().toISOString(); localStorage.setItem(storageKey, JSON.stringify({ version: 1, clips, settings, templateId, storySeed, updatedAt } satisfies Draft)); setSavedAt(updatedAt); }
  ...
  return <div className="flex min-h-[calc(100vh-4rem)] flex-col bg-background">
    ...
    <main className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)_280px]">
      <aside className="hidden border-r p-3 lg:block">{activeTemplate&&<><h2 className="mb-3 flex items-center gap-2 text-sm font-medium"><Sparkles className="size-4" />当前模板</h2><div className="mb-4 rounded-md border bg-muted/20 p-2.5"><p className="text-xs font-medium">{activeTemplate.name}</p><p className="mt-1 text-xs text-muted-foreground">{activeTemplate.description}</p></div></>}<h2 className="mb-3 flex items-center gap-2 text-sm font-medium"><Clapperboard className="size-4" />素材</h2><div className="space-y-2">{clips.map(c => <button key={c.id} onClick={() => setSelectedId(c.id)} className="flex w-full items-center gap-2 rounded-md border p-2 text-left hover:bg-muted"><Play className="size-3.5"/><span className="truncate text-xs">{c.title}</span></button>)}</div></aside>
