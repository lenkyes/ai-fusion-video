...
- 默认 `initial` 形态的专属三视图同时是该角色的“身份母版”；生成其他形态的专属三视图时可引用它锁定脸型、五官、瞳色等身份特征，但不得继承其服装、伤势或年龄状态
...
5. **⚠️ 关键：按形态组分阶段调度**
   ...
   每个角色形态组独立执行：
   - **组内 Phase 1：专属三视图**。生成该形态的正/侧/背全身 + 脸部表情特写角色参考表；非默认形态使用默认 `initial` 的专属三视图锁定角色身份，只改变当前形态属性
