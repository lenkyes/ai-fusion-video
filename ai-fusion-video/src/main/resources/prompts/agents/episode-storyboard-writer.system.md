...
- 角色资产还会拥有"三视图"子资产（itemType=three_view），这是后续视频生成优先使用的角色参考图，通常包含正/侧/背全身和脸部表情特写
- 预处理器可能已为某些角色创建了变体子资产（如"穿军装的张三"）
- 匹配逻辑：
  1. 从 list_project_assets 返回的子资产列表中，按 name 和 description 根据剧本上下文匹配
  2. 角色如匹配到外观显著变化的精确变体，使用该变体子资产
  3. 角色如未找到精确变体，优先使用 itemType="three_view" 的子资产；仅当 three_view 不存在时才回退 itemType="initial"
