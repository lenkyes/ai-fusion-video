...
5. **获取同组角色母版/参考图**：
   ...
   - 若当前目标是该组 `three_view`，优先根据形态根项的 `properties` / `aiPrompt`，再结合主资产信息生成该形态的参考母版。进入 `characterCanonicalMode` 时不传参考图；未进入该模式时，也只能把同组形态根项已有图片作为弱参考
   - 童年、青年、老年、换装、受伤等不同形态之间禁止互相引用图片；即使其他形态图片更完整也不得使用
...
| **角色 three_view** + 进入 characterCanonicalMode | 不传任何参考图，按 appearanceItem 的文字稳定锚点生成该形态的正/侧/背全身 + 脸部表情特写参考母版 | 无图片引用声明 | 正常纯文生图 |
...
* **角色严禁跨组引图**：角色引用图只能来自消息指定并校验通过的同一形态组。不得根据名称相似、排序位置或图片是否存在，自行选择其他 `initial`、`variant`、`age`、`costume`、`damaged` 或 `three_view`。
* **角色稳定性优先**：角色类图片不要把项目画风参考图作为 `imageUrls` 传入；角色只用文字画风约束。每个形态组的 `three_view` 是该形态自己的母版，形态根项主图必须从其专属三视图派生。
...
5. **稳定锚点来源优先级**：角色 prompt 必须优先使用 `appearanceItem.properties` 与 `appearanceItem.aiPrompt`，再使用目标 `item.properties`、`assetProperties`、`assetAiPrompt`、`assetDescription`。必须明确写出该形态的年龄阶段、脸型、眼睛形状、鼻梁、嘴唇、发型轮廓、体型、服装款式和主要配色；不得把其他形态的年龄、体型、发型或服装混入 prompt。缺失时用中性、端正、对称的国漫设定稿脸，不要自行加入怪异五官或夸张表情。
